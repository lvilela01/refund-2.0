...
export class DiskStorage {
  async saveFile(file: string) {
    ...
    // Garante que a pasta de uploads exista dentro da pasta tmp.
    await fs.promises.mkdir(uploadConfig.UPLOADS_FOLDER, { recursive: true })

    // rename é para mudar o arquivo de lugar, não renomear.
  ...
  async deleteFile(file: string, type: "tmp" | "upload") {
    ...
    try {
      await fs.promises.stat(filePath) // verifica o status do arquivo, se está disponível ou não.
    } catch {
      return
    }

    await fs.promises.unlink(filePath) // se passa pelo bloco de trycatch, deleta.
  }
}
